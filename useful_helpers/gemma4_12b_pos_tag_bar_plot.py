"""
Builds the gemma4_12b per-tag attention CSV (same schema/method as
data/bar_plot_csv/FrMaSc/{lov_7b,qvl_7b,lov_05b}.csv, via
data_plotting/make_bar_plot_csv_oidScaled.ipynb's extract_pos_tag_stats_to_csv,
copied verbatim) and renders the grouped normalized-attention bar plot (via
data_plotting/bar_plots_scaledOID.ipynb's plot_grouped_normalized_attention,
copied verbatim, same "# Gemma" cell style/merge_tags as the gemma3_4b_it plot)
against the global means already produced by fruit_math_pipeline.py.

Inputs (already on disk from the earlier vanilla run + tagging pipeline):
  npz_root:    $OTAT_RUN_ROOT/otat/gemma4_12b/FrMaSc
  json_root:   data/other_gemini_scaling_stuff/batch_tagging/FrMaSc_pos_tags/gemma4_12b/fruit_math
  global_csv:  $OTAT_RUN_ROOT/otat/gemma4_12b/FrMaSc/global_means/gemma4_12b__FrMaSc.csv

Output:
  data/bar_plot_csv/FrMaSc/gemma4_12b.csv
  data/plots/bar_plots/FrMaSc/gemma4_12b.png (+ .pdf)
"""
...
NPZ_ROOT = os.path.join(RUN_ROOT, "otat/gemma4_12b/FrMaSc")
JSON_ROOT = os.path.join(REPO_ROOT, "data/other_gemini_scaling_stuff/batch_tagging/FrMaSc_pos_tags/gemma4_12b/fruit_math")
GLOBAL_CSV = os.path.join(RUN_ROOT, "otat/gemma4_12b/FrMaSc/global_means/gemma4_12b__FrMaSc.csv")

PER_TAG_CSV = os.path.join(REPO_ROOT, "data/bar_plot_csv/FrMaSc/gemma4_12b.csv")
OUT_PLOT_PNG = os.path.join(REPO_ROOT, "data/plots/bar_plots/FrMaSc/gemma4_12b.png")
OUT_PLOT_PDF = os.path.join(REPO_ROOT, "data/plots/bar_plots/FrMaSc/gemma4_12b.pdf")
...
def main():
    print(f"Building per-tag CSV -> {PER_TAG_CSV}")
    extract_pos_tag_stats_to_csv(
        NPZ_ROOT, JSON_ROOT, CHUNKS_TO_PROCESS, PER_TAG_CSV,
    ...
    tag_order = ["SOG", "FRUIT_INTRO", "FRUIT_CONCEPT", "FIRST_HANDOFF", "OTHER_HANDOFF",
                 "MATH_INTRO", "MATH_ANSWER", "POSTAMBLE"]
    ...
    os.makedirs(os.path.dirname(OUT_PLOT_PNG), exist_ok=True)
    fig.savefig(OUT_PLOT_PNG, dpi=200, bbox_inches="tight")
    fig.savefig(OUT_PLOT_PDF, dpi=300, bbox_inches="tight")
    print(f"Wrote plot -> {OUT_PLOT_PNG}")
    print(f"Wrote plot -> {OUT_PLOT_PDF}")

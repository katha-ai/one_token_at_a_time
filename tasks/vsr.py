...
class VSRTask(BaseTask):
    def get_task_content(self, item):
        """
        VSR task content: caption + instruction.
        """
        caption = item['extended_caption_modify']
        ...
        # 2. Format chunks
        text_chunk = text_template.replace("[[caption]]", str(caption))
        inst_chunk = inst_template.replace("[[instruction]]", str(instruction))
